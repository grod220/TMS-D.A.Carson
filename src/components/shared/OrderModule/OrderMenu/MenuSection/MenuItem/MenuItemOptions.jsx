...
const MenuItemOptions = observer(({ store, options }) => {
  const handleChoice = (option, choice) => {
    ...
    if (choice.selection) {
      store.choices[option.name] = store.choices[option.name].filter(
      ...
      if (
        store.choices[option.name]
          .map(choiceobj => choiceobj.name)
          .includes(choice.name)
      ) {
        store.choices[option.name] = store.choices[option.name].filter(
          obj => obj.name !== choice.name
        );
      } else if (
        !option.maximum ||
        store.choices[option.name].filter(obj => obj.addition).length <
          option.maximum
      ) {
        store.choices[option.name].push(choice);
      }
  ...
  const verifySelected = (optionName, choiceName) => {
    return (
      store.choices[optionName] &&
      Boolean(
        store.choices[optionName].filter(obj => obj.name === choiceName).length
      )
    );
  };
  return options
        ...
        return (
          <div key={i}>
            <SectionHeader>
              {option.name}
              {option.maximum && (
                <MaxNotify>Choose {option.maximum} maximum</MaxNotify>
              )}
            </SectionHeader>
            <OptionsContainer>
              {option.choices.map((choice, i, arr) => {
                return (
                  <div key={i}>
                    {choice.addition && arr[i - 1] && arr[i - 1].selection && (
                      <Divider>----</Divider>
                    )}
